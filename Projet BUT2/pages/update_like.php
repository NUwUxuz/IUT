    ...
    $id_avis = $data['id_avis'];
        try {
            ...
            if (!empty($like_compte)) {    
                ...
                $stmt = $pdo->prepare('INSERT INTO pact._recommande (idC, idA) VALUES (:id_compte, :id_avis)');
                $stmt->execute(['id_compte' => $id_compte, 'id_avis' => $id_avis]);
            ...
            echo json_encode(['nbr_likes' => $result_like['nbr_likes'], 'nbr_dislikes' => $result_dislike['nbr_dislikes']]);
            exit;
